...
def create_mask_from_points(points, img_h, img_w):
    ...
    Returns:
        np.ndarray: Binary mask of shape (img_h, img_w), where 255 indicates inside the polygon.
    """
    # 创建全黑背景的二值掩码
...
# 创建 Laplace 核，只需构建一次即可重复使用
laplace_kernel = torch.tensor([[0, 1, 0], [1, -4, 1], [0, 1, 0]], dtype=torch.float32).unsqueeze(0).unsqueeze(0)
laplace_kernel = laplace_kernel.repeat(3, 1, 1, 1)  # 重复以匹配 RGB 三通道

def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask, box_fore, box_blend):
    """
    计算前景和融合图像在遮罩区域内的拉普拉斯损失。
    
    Args:
        foreground_img (torch.Tensor): 前景图像张量，形状为 (B, C, H, W)。
        foreground_mask (torch.Tensor): 前景遮罩张量，形状为 (B, 1, H, W)。
        blended_img (torch.Tensor): 融合图像张量，形状为 (B, C, H, W)。
        background_mask (torch.Tensor): 背景遮罩张量，形状为 (B, 1, H, W)。
        box_fore (torch.Tensor): 前景图像裁剪框坐标，形状为 (2, 2)。
        box_blend (torch.Tensor): 融合图像裁剪框坐标，形状为 (2, 2)。

    Returns:
        torch.Tensor: 计算出的拉普拉斯损失。
    """
    device = foreground_img.device
    laplace_kernel_device = laplace_kernel.to(device)
    
    # 计算前景和融合图像的拉普拉斯
    fore_laplace = F.conv2d(foreground_img, laplace_kernel_device, stride=1, padding=1, groups=3)
    blend_laplace = F.conv2d(blended_img, laplace_kernel_device, stride=1, padding=1, groups=3)
    
    # 应用前景和背景遮罩，确保与图像通道匹配
    fore_laplace_masked = fore_laplace * foreground_mask
    blend_laplace_masked = blend_laplace * background_mask

    # 计算裁剪框内的拉普拉斯差异
    loss_area_fore = fore_laplace_masked[..., box_fore[0, 0]:box_fore[1, 0], box_fore[0, 1]:box_fore[1, 1]]
    loss_area_blend = blend_laplace_masked[..., box_blend[0, 0]:box_blend[1, 0], box_blend[0, 1]:box_blend[1, 1]]
    laplace_loss = torch.sum(torch.abs(loss_area_fore - loss_area_blend))
    
    # 归一化损失值，防止除零
    laplace_loss /= (torch.sum(foreground_mask) + 1e-8)  # 避免除以零
